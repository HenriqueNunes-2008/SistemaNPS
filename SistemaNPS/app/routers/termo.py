...
def salvar_termo(data: TermoRequest, request: Request):
    try:
        if not is_admin_mode_request(request):
            raise HTTPException(status_code=403, detail="Somente o administrador pode preencher o termo.")
        if data.termo_dados:
...
def atualizar_termo(data: TermoUpdateRequest, request: Request):
    try:
        ...
        if _is_user_edit_locked(proc) and not is_admin:
            ...
            # O cliente somente acrescenta sua aprovação; os demais dados do
            # termo, inclusive fotos, são sempre preservados do registro salvo.
            aprovacao = data.termo_dados.get("aprovacao") or {}
            data.termo_dados = dict(dados_atuais)
            data.termo_dados["aprovacao"] = aprovacao
            data.imagens = []
            data.cpf = proc.get("cpf") or data.cpf
            data.nome_cliente = proc.get("nome_cliente") or data.nome_cliente
            data.empresa = proc.get("empresa")
            data.status_entrega = proc.get("status_entrega") or data.status_entrega
        ...
        if is_admin and data.termo_dados:
            ...
            if dados_originais:
                assinaturas_originais = dados_originais.get("assinaturas") or {}
                assinaturas_recebidas = data.termo_dados.get("assinaturas") or {}
                representante = assinaturas_recebidas.get("representante") or {}
                data.termo_dados = dict(dados_originais)
                data.termo_dados["assinaturas"] = dict(assinaturas_originais)
                data.termo_dados["assinaturas"]["representante"] = representante
                data.cpf = proc.get("cpf") or data.cpf
                data.nome_cliente = proc.get("nome_cliente") or data.nome_cliente
                data.empresa = proc.get("empresa")
                data.status_entrega = proc.get("status_entrega") or data.status_entrega
